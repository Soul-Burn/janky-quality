for _, qm in pairs(lib.quality_modules) do
    lib.add_prototype({ name = lib.name_with_quality_module("basic-solid", 3, qm), type = "resource-category" })
end
...
for _, resource in pairs(data.lib.resource) do
    if resource.category == "basic-solid" then
        for _, qm in pairs(lib.quality_modules) do
            ...
            new_resource.name = lib.name_with_quality_module(resource.name, 3, qm)
            ...
            new_resource.category = lib.name_with_quality_module("basic-solid", 3, qm)
